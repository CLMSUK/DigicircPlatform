...
using DigicircMatchmaking.Security.Encryption;
...
namespace AppCode
{
    public class BaseViewPage<T> : System.Web.Mvc.WebViewPage<T>
    {
        public static bool IsGoogleAnalyticsEnabled
        {
            get
            {
                return GoogleAnalyticsTags.Count > 0;
            }
        }

        private static List<string> _googleAnalyticTags;

        public static List<string> GoogleAnalyticsTags
        {
            get
            {
                if (_googleAnalyticTags == null)
                {
                    var setting = zAppDev.DotNet.Framework.Utilities.Common.GetConfigurationKey("GoogleAnalyticsTags");
                    if (setting == null)
                    {
                        _googleAnalyticTags = new List<string>();
                    }
                    else
                    {
                        _googleAnalyticTags = setting.ToString().Split(';').ToList();
                    }
                }
                return _googleAnalyticTags;
            }
        }

        public static string GoogleTags => string.Join("", GoogleAnalyticsTags.Select(t => $"gtag('config', '{t}'); "));
        ...
        public void Init(string formName)
        {
            ...
            //Init is being called every time a page loaded. Why not set the current language in the Service as well,
            //so as to minimize database reads
            //ServiceLocator.Current.GetInstance<ILocalesService>().SetCurrentLanguage(_lang);
        }
