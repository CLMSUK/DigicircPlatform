...
interface Window { scope_BusinessFunctionForm: DigicircMatchmaking.Controllers.IBusinessFunctionFormScope; }
namespace DigicircMatchmaking.Controllers {
const BusinessFunctionFormConditionalFormattings = [
new Joove.JbRule({
...
condition: async (_parents) => {
        return Joove.Common.nullSafe<any>(() => CLMS.Framework.String.IsNullOrEmpty(Joove.Common.nullSafe(function () {
            return Joove.Common.nullSafe<any>(() => window["scope_BusinessFunctionForm"].model.Title, "").trim();
        }, "")), false);
...
const BusinessFunctionFormDataValidations = [
        ];
const BusinessFunctionFormCalculatedExpressions = [
        ];
...
export interface IBusinessFunctionFormScope extends Joove.IWebPageScope {
model:
    DigicircMatchmaking.ViewModels.BusinessFunctionForm.BusinessFunctionFormViewModel;
...
class BusinessFunctionFormController {
...
private timeoutDelayLockcmdCompanySaveClicked:
    ...
    private controllerActionFinished = (resetDirty: boolean) => {
        //window.scope_BusinessFunctionForm.model = window.scope_BusinessFunctionForm.dehydrate();
        //window.scope_MasterPage.model = window.scope_BusinessFunctionForm.model;
        DigicircMatchmaking.ViewModels.BusinessFunctionForm.BusinessFunctionFormViewModel._lightCast(window.scope_BusinessFunctionForm.model);
        // Joove.Common.applyScope(window.scope_BusinessFunctionForm);
        if (resetDirty) {
    ...
    constructor(private $scope: IBusinessFunctionFormScope, $timeout: ng.ITimeoutService) {
        window.scope_BusinessFunctionForm = $scope;
        $scope.$onControlChanged = function (event, newValue, dontMakeDirty) {
            Joove.Core.onChange(event.target, newValue, dontMakeDirty)
        }

        $scope.dehydrate = (context = null) => DigicircMatchmaking.ViewModels.BusinessFunctionForm.BusinessFunctionFormViewModel._initializeFrom($scope.model, context);
    ...
    init($scope: IBusinessFunctionFormScope, $timeout: ng.ITimeoutService) {


        $scope.model = DigicircMatchmaking.ViewModels.BusinessFunctionForm.BusinessFunctionFormViewModel._initializeFrom(window.viewDTO.Model);
...
Add:
            async (_cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("BusinessFunctionForm", "Add", "GET", [], {}, _modalOptions);
            },
Edit:
            async (id, _cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("BusinessFunctionForm", "Edit", "GET", [id], {}, _modalOptions);
            },
Save:
            async (_cb, _modalOptions, _e) => {
                Joove.Common.autocompleteFix();
...
verb: 'POST', controller: 'BusinessFunctionForm', action: 'Save',
...
Delete:
            async (_cb, _modalOptions, _e) => {
                ...
                window._context.isDirty = false;
                Joove.Core.executeControllerActionNew({
verb: 'POST', controller: 'BusinessFunctionForm', action: 'Delete',
queryData: [], postData: {'model': $scope.model},
cb: _cb, modalOptions: _modalOptions, event: _e
                });
            },
...
// Rules
        window._ruleEngine.addDataValidations(BusinessFunctionFormDataValidations);
        window._ruleEngine.addConditionalFormattings(BusinessFunctionFormConditionalFormattings);
        window._ruleEngine.addCalculatedExpressions(BusinessFunctionFormCalculatedExpressions);
...
angular.module("Application").controller("BusinessFunctionFormController", ["$scope", "$timeout", BusinessFunctionFormController] as Array<string>);
}
