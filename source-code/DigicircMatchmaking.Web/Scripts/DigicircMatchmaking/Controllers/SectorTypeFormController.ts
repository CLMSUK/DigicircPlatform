...
interface Window { scope_SectorTypeForm: DigicircMatchmaking.Controllers.ISectorTypeFormScope; }
namespace DigicircMatchmaking.Controllers {
const SectorTypeFormConditionalFormattings = [
new Joove.JbRule({
...
condition: async (_parents) => {
        return Joove.Common.nullSafe<any>(() => CLMS.Framework.String.IsNullOrEmpty(Joove.Common.nullSafe(function () {
            return Joove.Common.nullSafe<any>(() => window["scope_SectorTypeForm"].model.Title, "").trim();
        }, "")), false);
...
const SectorTypeFormDataValidations = [
                                      ];
const SectorTypeFormCalculatedExpressions = [
        ];
...
export interface ISectorTypeFormScope extends Joove.IWebPageScope {
model:
    DigicircMatchmaking.ViewModels.SectorTypeForm.SectorTypeFormViewModel;
...
class SectorTypeFormController {
...
private timeoutDelayLockcmdCompanySaveClicked:
    ...
    private controllerActionFinished = (resetDirty: boolean) => {
        //window.scope_SectorTypeForm.model = window.scope_SectorTypeForm.dehydrate();
        //window.scope_MasterPage.model = window.scope_SectorTypeForm.model;
        DigicircMatchmaking.ViewModels.SectorTypeForm.SectorTypeFormViewModel._lightCast(window.scope_SectorTypeForm.model);
        // Joove.Common.applyScope(window.scope_SectorTypeForm);
        if (resetDirty) {
    ...
    constructor(private $scope: ISectorTypeFormScope, $timeout: ng.ITimeoutService) {
        window.scope_SectorTypeForm = $scope;
        $scope.$onControlChanged = function (event, newValue, dontMakeDirty) {
            Joove.Core.onChange(event.target, newValue, dontMakeDirty)
        }

        $scope.dehydrate = (context = null) => DigicircMatchmaking.ViewModels.SectorTypeForm.SectorTypeFormViewModel._initializeFrom($scope.model, context);
    ...
    init($scope: ISectorTypeFormScope, $timeout: ng.ITimeoutService) {


        $scope.model = DigicircMatchmaking.ViewModels.SectorTypeForm.SectorTypeFormViewModel._initializeFrom(window.viewDTO.Model);
...
Add:
            async (_cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("SectorTypeForm", "Add", "GET", [], {}, _modalOptions);
            },
Edit:
            async (id, _cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("SectorTypeForm", "Edit", "GET", [id], {}, _modalOptions);
            },
Save:
            async (_cb, _modalOptions, _e) => {
                Joove.Common.autocompleteFix();
...
verb: 'POST', controller: 'SectorTypeForm', action: 'Save',
...
Delete:
            async (_cb, _modalOptions, _e) => {
                ...
                window._context.isDirty = false;
                Joove.Core.executeControllerActionNew({
verb: 'POST', controller: 'SectorTypeForm', action: 'Delete',
queryData: [], postData: {'model': $scope.model},
cb: _cb, modalOptions: _modalOptions, event: _e
                });
            },
...
// Rules
        window._ruleEngine.addDataValidations(SectorTypeFormDataValidations);
        window._ruleEngine.addConditionalFormattings(SectorTypeFormConditionalFormattings);
        window._ruleEngine.addCalculatedExpressions(SectorTypeFormCalculatedExpressions);
...
angular.module("Application").controller("SectorTypeFormController", ["$scope", "$timeout", SectorTypeFormController] as Array<string>);
}
