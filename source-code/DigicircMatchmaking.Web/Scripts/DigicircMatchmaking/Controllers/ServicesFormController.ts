...
interface Window { scope_ServicesForm: DigicircMatchmaking.Controllers.IServicesFormScope; }
namespace DigicircMatchmaking.Controllers {
const ServicesFormConditionalFormattings = [
new Joove.JbRule({
...
condition: async (_parents) => {
        return Joove.Common.nullSafe<any>(() => CLMS.Framework.String.IsNullOrEmpty(Joove.Common.nullSafe(function () {
            return Joove.Common.nullSafe<any>(() => window["scope_ServicesForm"].model.Title, "").trim();
        }, "")), false);
...
const ServicesFormDataValidations = [
                                    ];
const ServicesFormCalculatedExpressions = [
        ];
...
export interface IServicesFormScope extends Joove.IWebPageScope {
model:
    DigicircMatchmaking.ViewModels.ServicesForm.ServicesFormViewModel;
...
private timeoutDelayLockcmdCompanySaveClicked:
    ...
    private controllerActionFinished = (resetDirty: boolean) => {
        //window.scope_ServicesForm.model = window.scope_ServicesForm.dehydrate();
        //window.scope_MasterPage.model = window.scope_ServicesForm.model;
        DigicircMatchmaking.ViewModels.ServicesForm.ServicesFormViewModel._lightCast(window.scope_ServicesForm.model);
        // Joove.Common.applyScope(window.scope_ServicesForm);
        if (resetDirty) {
    ...
    constructor(private $scope: IServicesFormScope, $timeout: ng.ITimeoutService) {
        window.scope_ServicesForm = $scope;
        $scope.$onControlChanged = function (event, newValue, dontMakeDirty) {
            Joove.Core.onChange(event.target, newValue, dontMakeDirty)
        }

        $scope.dehydrate = (context = null) => DigicircMatchmaking.ViewModels.ServicesForm.ServicesFormViewModel._initializeFrom($scope.model, context);
    ...
    init($scope: IServicesFormScope, $timeout: ng.ITimeoutService) {


        $scope.model = DigicircMatchmaking.ViewModels.ServicesForm.ServicesFormViewModel._initializeFrom(window.viewDTO.Model);
...
Add:
            async (_cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("ServicesForm", "Add", "GET", [], {}, _modalOptions);
            },
Edit:
            async (id, _cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("ServicesForm", "Edit", "GET", [id], {}, _modalOptions);
            },
Save:
            async (_cb, _modalOptions, _e) => {
                Joove.Common.autocompleteFix();
...
verb: 'POST', controller: 'ServicesForm', action: 'Save',
...
Delete:
            async (_cb, _modalOptions, _e) => {
                ...
                window._context.isDirty = false;
                Joove.Core.executeControllerActionNew({
verb: 'POST', controller: 'ServicesForm', action: 'Delete',
queryData: [], postData: {'model': $scope.model},
cb: _cb, modalOptions: _modalOptions, event: _e
                });
            },
...
// Rules
        window._ruleEngine.addDataValidations(ServicesFormDataValidations);
        window._ruleEngine.addConditionalFormattings(ServicesFormConditionalFormattings);
        window._ruleEngine.addCalculatedExpressions(ServicesFormCalculatedExpressions);
...
angular.module("Application").controller("ServicesFormController", ["$scope", "$timeout", ServicesFormController] as Array<string>);
}
