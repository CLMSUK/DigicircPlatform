...
interface Window { scope_ProcessForm: DigicircMatchmaking.Controllers.IProcessFormScope; }
namespace DigicircMatchmaking.Controllers {
const ProcessFormConditionalFormattings = [
new Joove.JbRule({
...
condition: async (_parents) => {
        return Joove.Common.nullSafe<any>(() => CLMS.Framework.String.IsNullOrEmpty(Joove.Common.nullSafe(function () {
            return Joove.Common.nullSafe<any>(() => window["scope_ProcessForm"].model.Title, "").trim();
        }, "")), false);
...
const ProcessFormDataValidations = [
                                   ];
const ProcessFormCalculatedExpressions = [
        ];
...
export interface IProcessFormScope extends Joove.IWebPageScope {
model:
    DigicircMatchmaking.ViewModels.ProcessForm.ProcessFormViewModel;
...
private timeoutDelayLockcmdCompanySaveClicked:
    ...
    private controllerActionFinished = (resetDirty: boolean) => {
        //window.scope_ProcessForm.model = window.scope_ProcessForm.dehydrate();
        //window.scope_SymbiosisMasterPage.model = window.scope_ProcessForm.model;
        DigicircMatchmaking.ViewModels.ProcessForm.ProcessFormViewModel._lightCast(window.scope_ProcessForm.model);
        // Joove.Common.applyScope(window.scope_ProcessForm);
        if (resetDirty) {
    ...
    constructor(private $scope: IProcessFormScope, $timeout: ng.ITimeoutService) {
        window.scope_ProcessForm = $scope;
        $scope.$onControlChanged = function (event, newValue, dontMakeDirty) {
            Joove.Core.onChange(event.target, newValue, dontMakeDirty)
        }

        $scope.dehydrate = (context = null) => DigicircMatchmaking.ViewModels.ProcessForm.ProcessFormViewModel._initializeFrom($scope.model, context);
    ...
    init($scope: IProcessFormScope, $timeout: ng.ITimeoutService) {


        $scope.model = DigicircMatchmaking.ViewModels.ProcessForm.ProcessFormViewModel._initializeFrom(window.viewDTO.Model);
...
Add:
            async (_cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("ProcessForm", "Add", "GET", [], {}, _modalOptions);
            },
Edit:
            async (id, _cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("ProcessForm", "Edit", "GET", [id], {}, _modalOptions);
            },
Save:
            async (_cb, _modalOptions, _e) => {
                Joove.Common.autocompleteFix();
...
verb: 'POST', controller: 'ProcessForm', action: 'Save',
...
Delete:
            async (_cb, _modalOptions, _e) => {
                ...
                window._context.isDirty = false;
                Joove.Core.executeControllerActionNew({
verb: 'POST', controller: 'ProcessForm', action: 'Delete',
queryData: [], postData: {'model': $scope.model},
cb: _cb, modalOptions: _modalOptions, event: _e
                });
            },
...
// Rules
        window._ruleEngine.addDataValidations(ProcessFormDataValidations);
        window._ruleEngine.addConditionalFormattings(ProcessFormConditionalFormattings);
        window._ruleEngine.addCalculatedExpressions(ProcessFormCalculatedExpressions);
...
angular.module("Application").controller("ProcessFormController", ["$scope", "$timeout", ProcessFormController] as Array<string>);
}
