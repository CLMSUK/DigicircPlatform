...
interface Window { scope_CountryForm: DigicircMatchmaking.Controllers.ICountryFormScope; }
namespace DigicircMatchmaking.Controllers {
const CountryFormConditionalFormattings = [
new Joove.JbRule({
...
condition: async (_parents) => {
        return Joove.Common.nullSafe<any>(() => CLMS.Framework.String.IsNullOrEmpty(Joove.Common.nullSafe(function () {
            return Joove.Common.nullSafe<any>(() => window["scope_CountryForm"].model.Title, "").trim();
        }, "")), false);
...
const CountryFormDataValidations = [
                                   ];
const CountryFormCalculatedExpressions = [
        ];
...
export interface ICountryFormScope extends Joove.IWebPageScope {
model:
    DigicircMatchmaking.ViewModels.CountryForm.CountryFormViewModel;
...
private timeoutDelayLockcmdCompanySaveClicked:
    ...
    private controllerActionFinished = (resetDirty: boolean) => {
        //window.scope_CountryForm.model = window.scope_CountryForm.dehydrate();
        //window.scope_MasterPage.model = window.scope_CountryForm.model;
        DigicircMatchmaking.ViewModels.CountryForm.CountryFormViewModel._lightCast(window.scope_CountryForm.model);
        // Joove.Common.applyScope(window.scope_CountryForm);
        if (resetDirty) {
    ...
    constructor(private $scope: ICountryFormScope, $timeout: ng.ITimeoutService) {
        window.scope_CountryForm = $scope;
        $scope.$onControlChanged = function (event, newValue, dontMakeDirty) {
            Joove.Core.onChange(event.target, newValue, dontMakeDirty)
        }

        $scope.dehydrate = (context = null) => DigicircMatchmaking.ViewModels.CountryForm.CountryFormViewModel._initializeFrom($scope.model, context);
    ...
    init($scope: ICountryFormScope, $timeout: ng.ITimeoutService) {


        $scope.model = DigicircMatchmaking.ViewModels.CountryForm.CountryFormViewModel._initializeFrom(window.viewDTO.Model);
...
Add:
            async (_cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("CountryForm", "Add", "GET", [], {}, _modalOptions);
            },
Edit:
            async (id, _cb, _modalOptions, _e) => {
                ...
                Joove.Core.executeRedirectControllerAction("CountryForm", "Edit", "GET", [id], {}, _modalOptions);
            },
Save:
            async (_cb, _modalOptions, _e) => {
                Joove.Common.autocompleteFix();
...
verb: 'POST', controller: 'CountryForm', action: 'Save',
...
Delete:
            async (_cb, _modalOptions, _e) => {
                ...
                window._context.isDirty = false;
                Joove.Core.executeControllerActionNew({
verb: 'POST', controller: 'CountryForm', action: 'Delete',
queryData: [], postData: {'model': $scope.model},
cb: _cb, modalOptions: _modalOptions, event: _e
                });
            },
...
// Rules
        window._ruleEngine.addDataValidations(CountryFormDataValidations);
        window._ruleEngine.addConditionalFormattings(CountryFormConditionalFormattings);
        window._ruleEngine.addCalculatedExpressions(CountryFormCalculatedExpressions);
...
angular.module("Application").controller("CountryFormController", ["$scope", "$timeout", CountryFormController] as Array<string>);
}
