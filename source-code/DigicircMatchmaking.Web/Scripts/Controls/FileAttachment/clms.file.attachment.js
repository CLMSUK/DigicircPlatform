                ...
                link: function ($scope, $element, $attrs, ngModelCtrl) {
                    ...
                    $scope.remove = function ($anchor) {
                        ...
                        Joove.Core.executeControllerAction(Joove.Core.getControllerForElement($element, false), name + "_Remove", "POST", [], { model: Joove.Common.getModel(), indexes: indexes.key }, null, function (data) {
                            ...
                            if ($scope.isMultiple) {
                                var deletedFile = $scope.previousModelFileNames[indexes.indexes[0]];
                    ...
                    function init(isAdvancedUpload) {
                        ...
                        if ($scope.isMultiple === true) {
                            $scope.$watchCollection("model", function (value) {
                                if (($scope.previousModelFileNames != null && $scope.previousModelFileNames.length) && ($scope.previousModelFileNames.length > value.length)) {
                                    if ($scope.deletingFile === true)
                                        return;
                                    for (var i = 0; i < $scope.previousModelFileNames.length; i++) {
                                        var item = $scope.previousModelFileNames[i];
                                        var foundItem = false;
                                ...
                                $scope.previousModelFileNames = new Array();
                                if (value != null) {
                                    for (var x = 0; x < value.length; x++) {
