...
using DigicircMatchmaking.Security.Owin;
...
namespace DigicircMatchmaking.Web
{
    public class Startup
    {
        public void ConfigureServices(IAppBuilder app, System.Web.Http.HttpConfiguration config)
        {
            ...
            .SingleInstance();
            builder
            .RegisterInstance(ExposedServiceLogger.FromConfiguration())
            .As<IExposedServiceLogger>()
            .SingleInstance();
            builder
            ...
            .SingleInstance();
            builder
            .Register(c => new DigicircMatchmaking.Hubs.EventsHub())
            ...
            .Register(c =>
            {
                ...
                scheduleManager.SetSchedules(new List<zAppDev.DotNet.Framework.Workflow.WorkflowSchedule> { new zAppDev.DotNet.Framework.Workflow.WorkflowSchedule {
                        Workflow = "QueryGeocoder",
                        Active = true,
                        StartDateTime = DateTime.ParseExact("01/01/2020 03:08","dd/MM/yyyy HH:mm",System.Globalization.CultureInfo.InvariantCulture),
                        ExpireOn = DateTime.ParseExact("01/01/2021 03:09","dd/MM/yyyy HH:mm", System.Globalization.CultureInfo.InvariantCulture),
                        CronExpression =  "0 1 * * *",
                        CronExpressionTimezone = ConfigurationManager.AppSettings["DefaultTimezoneId"]
                    },new zAppDev.DotNet.Framework.Workflow.WorkflowSchedule {
                        Workflow = "SuggestionsFlow",
                        Active = true,
                        StartDateTime = DateTime.ParseExact("01/03/2021 04:40","dd/MM/yyyy HH:mm",System.Globalization.CultureInfo.InvariantCulture),
                        ExpireOn = DateTime.ParseExact("01/03/2026 04:44","dd/MM/yyyy HH:mm", System.Globalization.CultureInfo.InvariantCulture),
                        CronExpression =  "0 * * * *",
                        CronExpressionTimezone = ConfigurationManager.AppSettings["DefaultTimezoneId"]
                    }
                });
            ...
            .Register(c =>
            {
                zAppDev.DotNet.Framework.Tools.PerformanceMeasurements.Configuration.PerformanceMonitorConfiguration monitorConfiguration = null;
                monitorConfiguration = new zAppDev.DotNet.Framework.Tools.PerformanceMeasurements.Configuration.PerformanceMonitorConfiguration();
            ...
            .Register(c =>
            {
                return zAppDev.DotNet.Framework.Data.DatabaseManagers.DatabaseUtilities.CommonUtilities.CreateDatabaseManager();
            })
            .As<IDatabaseManager>()
            .SingleInstance();
        ...
        public void Configuration(IAppBuilder app)
        {
            ...
            Identity.UserManager.ReadPasswordPolicyFromConfiguration(ConfigurationManager.AppSettings);
            ...
            {
                log4net.LogManager.GetLogger("CookieValidationInterval")
                .Error("Could not parse Cookie Validation Interval Setting! Using default...", e);
            }
            ...
            {
                log4net.LogManager.GetLogger("SessionStateStorage")
                .Error("Could not retrieve cache configuration for Session Storage!", e);
            }
            log4net.LogManager.GetLogger("AuthenticationCookie")
            .Info($"Authentication Cookie Timeout: {authenticationCookineExpirationTimeout.Minutes} minute(s)");
            ...
            ServiceLocator.Current.GetInstance<IDatabaseManager>().UpdateApplicationSettingsTable();
            DigicircMatchmaking.DatabaseSeeder databaseSeeder = new DigicircMatchmaking.DatabaseSeeder();
            ...
            .Init(typeof(DigicircMatchmaking.DAL.Repository).Assembly);
            DigicircMatchmaking.Hubs.EventsHub.RaiseApplicationStart();
        ...
        private void ConfigeAuditTrailManager()
        {
            ...
            if (!enabled)
            {
                ...
                .Info(@"Audit Trail is not enabled. Skipping initialization. If you with to enable it,
please add 'audittrail:enabled' in the application settings of web.config.");
                return;
            }
