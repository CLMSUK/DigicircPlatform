...
using DigicircMatchmaking.UI.ViewModels.ManageUser;
...
namespace DigicircMatchmaking.UI.Controllers.ManageUser
{
    ...
    public class ManageUserController : ControllerBase<ManageUserViewModel, ManageUserViewModelDTO>
    {
        ...
        public ManageUserController SetModel(ManageUserViewModel m)
        {
            @model = m;
            return this;
        }
        ...
        public ActionResult ExecuteEditUser(string Id)
        {
            ...
            ((MasterPageForSlide.MasterPageForSlideController) _parentController).ExecuteRender();
            ActionResult _result = null;
            @model.ApplicationUser =  new DigicircMatchmaking.DAL.Repository().GetById<zAppDev.DotNet.Framework.Identity.Model.ApplicationUser>(Id);
        ...
        {
            ActionResult _result = null;
            new DigicircMatchmaking.DAL.Repository().DeleteApplicationUser(@model.ApplicationUser);
            _result = CloseForm();
            return _result;
        }
        ...
        public JsonResult Button1_PickList_Datasource()
        {
            ...
            if (datasourceRequest.GroupBy.Any())
            {
                ...
                response.Data = items.ToList().Select(i => new Button1DataSet_ApplicationPermissionDTO(i, true)).ToList();
...
//Fix for total items in GetGroupsClosed
        ...
        public JsonResult Button2_PickList_Datasource()
        {
            ...
            if (datasourceRequest.GroupBy.Any())
            {
                var groups = DatasourceRetriever.RetrieveGrouped(datasourceRequest, queryable, q => q.Id, postedData);
                var items = groups.GetAllItems();
                response.Data = items.ToList().Select(i => new Button2DataSet_ApplicationRoleDTO(i, true)).ToList();
                response.Groups = groups;
//Fix for total items in GetGroupsClosed
                if (datasourceRequest.GroupBy.FirstOrDefault().GetGroupsClosed)
                {
